...
This project now uses [uv](https://github.com/astral-sh/uv) for dependency and
...
# Create or update the local .venv and install dependencies
uv sync
```

All commands below can be executed with `uv run ...` to ensure the managed
environment is used. The legacy `requirements*.txt` files are kept for reference
only and are no longer the primary install path.
...
The PyTorch-based script uses an ImageNet-pretrained ResNet18 to produce 128-D
embeddings by default. To generate features from standard MOT challenge
detections, assuming resources have been extracted to the repository root
directory and MOT16 data is in `./MOT16`:
```
uv run python tools/generate_detections.py \
    --mot_dir=./MOT16/train \
    --output_dir=./resources/detections/MOT16_train
```
- Omit `--model` to use the default ResNet18 encoder.
- Provide `--model=/path/to/state_dict.pt` to load a custom PyTorch checkpoint
  (expects the same 128-D projection layer).
- Use `--device` to select `cpu`, `cuda`, or `mps` (defaults to auto-detection).
...
**Legacy TensorFlow note:** TensorFlow-specific scripts (e.g., `tools/freeze_model.py`)
are kept for reference only and are not installed by default. Use the PyTorch
encoder above for new work.

## Training on MOT16 (appearance model)

1) Download MOT16 from the official site ([link](https://motchallenge.net/data/MOT16/)) and place it at `./MOT16` so that images live under `./MOT16/train/*/img1`.

2) Start training (TensorBoard logging by default):
```
uv run python tools/train_mot16.py \
    --data-root ./MOT16 \
...
3) Visualize logs:
```
uv run tensorboard --logdir ./runs/mot16
```

4) To log to a local/connected Weights & Biases instance instead of (or in addition to) TensorBoard:
```
uv run python tools/train_mot16.py \
    --data-root ./MOT16 \
    --output-dir ./checkpoints/mot16 \
...
```
Checkpoints are saved after every epoch in `--output-dir`.
