...
def run_tracker(sequence: str, detections_dir: Path, tracks_dir: Path, mot_dir: Path) -> None:
    """Run Deep SORT tracking using DINOv2-based detections."""
    env = os.environ.copy()
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run tracker using DINOv2 detections.")
    parser.add_argument("--sequence", required=True, help="Sequence name, e.g., MOT16-02")
    parser.add_argument(
        "--detections_dir",
        type=Path,
        default=ROOT / "resources" / "detections_dinov2",
        help="Directory containing DINOv2-based detection .npy files.",
    )
    parser.add_argument("--tracks_dir", type=Path, default=ROOT / "tracks_dinov2")
